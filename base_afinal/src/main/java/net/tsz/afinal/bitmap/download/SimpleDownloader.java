...
public class SimpleDownloader implements Downloader {
	...
	public byte[] download (String urlString, OnCompleteListener lis){
		if (urlString == null)
			return null;
		if (urlString.trim().toLowerCase().startsWith("http")) {
			return getFromHttp(urlString, lis);
			...
			try {
				...
				Log.e(TAG, "Error in read from file - " + urlString);
...
//			Log.e(TAG, "Error in read from file - " + file + " : " + e);
	...
	private byte[] getFromHttp(String urlString, OnCompleteListener lis) {
		...
		try {
			...
			while ((b = in.read(buff)) != -1) {
				baos.write(buff, 0, b);
				if (lis != null) {
					lis.onDowning(urlString, baos.size(), len);
				}
			}
			return baos.toByteArray();
		} catch (final IOException e) {
			Log.e(TAG, "Error in downloadBitmap - " + urlString);
			if (lis != null) {
				lis.onDowning(urlString, -1, -1);
			}
//			Log.e(TAG, "Error in downloadBitmap - " + urlString + " : " + e);
		} catch (final OutOfMemoryError e) {
			Log.e(TAG, "OOM in downloadBitmap - " + urlString);
			if (lis != null) {
				lis.onDowning(urlString, -1, -1);
			}
//			Log.e(TAG, "Error in downloadBitmap - " + urlString + " : " + e);
